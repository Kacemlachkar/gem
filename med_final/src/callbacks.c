...
{
GtkWidget *ajouterobs;
GtkWidget *afficherobs;
GtkWidget *obs_table;

ajouterobs=lookup_widget(objet, "ajouterobs");

gtk_widget_destroy(ajouterobs);
afficherobs=lookup_widget(objet, "afficherobs");
afficherobs=create_afficherobs();

gtk_widget_show(afficherobs);

obs_table=lookup_widget(afficherobs, "obs_table");
...
gtk_widget_destroy(afficherobs);
...
on_obs_actualiser_clicked              (GtkWidget       *objet,
                                        gpointer         user_data)
{
GtkWidget *afficherobs,*w1;
GtkWidget *obs_table;

w1=lookup_widget(objet, "afficherobs");
afficherobs=create_afficherobs();
...
GtkWidget *statistiquesobs,*total,*tauxobse,*tauxobsn;
GtkWidget *afficherobs;
char n[40];
char k[40];
char g[40];
...
statistiquesobs=lookup_widget(objet, "statistiquesobs");


statistiquesobs=create_statistiquesobs();

gtk_widget_show(statistiquesobs);
total=lookup_widget(statistiquesobs, "total");
tauxobse=lookup_widget(statistiquesobs, "tauxobse");
tauxobsn=lookup_widget(statistiquesobs, "tauxobsn");
	nb=nbobservateur();
	sprintf(n,"%d",nb);
